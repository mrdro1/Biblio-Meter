...
def title_to_vector(title):
    ...
    """
    filtered_title = ''.join(filter(lambda x: str.isalpha(x) or str.isspace(x), title))
    result_vector = ''.join(filter(lambda x: len(x) >= 4, filtered_title.split()))
...
def get_DOI_by_title(title):
    ...
    try:
        ...
        hrefs = [x for x in soup.find_all('a', class_='cite-link', href=True) if x['href'] != '#']
