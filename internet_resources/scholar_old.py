        ...
        while result and soup is None:
            ...
            if soup is None:
                ...
                while result is None:
                    result = input(
                        'Do not load cluster page on scholar. Try again? [Y/N]').lower()
                    if result == "y":
                        result = True
                    elif result == "n":
                        result = False
...
def _cluster_handler(cluster_id, papers_count):
    logger.debug(
        "Handle %i papers from cluster %s." %
        ...
        for links in footer_links:
            if links != {}:
                ...
                if not "year" in paper_EndNote_data or not "author" in paper_EndNote_data:
                    ...
                    similar_papers = intersect_papers(
                        paper_EndNote_data, EndNote_list)
                    if similar_papers == []:
                        merged_counter += 1
                        logger.debug(
                            ...
                            file_counter)
                        logger.debug(
                            "Add EndNote file #%i in merged array." %
                            file_counter)
    ...
    if merged_counter == 0:
        ...
        logger.debug(
            "All {0} EndNote files merged in {1} (i.e. distinct versions in cluster: {1}):".format(
                file_counter, merged_counter))
        for counter, data in enumerate(EndNote_list):
            logger.debug(
                "Merged EndNote file #%i:\n%s" %
...
def _get_info_from_resulting_selection(paper_soup, handling_cluster=False):
    ...
    if handling_cluster and general_information["cluster"] is not None:
        ...
        if different_information is not None:
            ...
            settings.print_message(
                "Versions in cluster: %i." %
                len(different_information), 3)
            return full_info
...
def get_info_from_EndNote(file_url, return_source=False):
    ...
    while result and EndNode_file is None:
        ...
        if EndNode_file is None:
            ...
            while result is None:
                result = input(
                    'Do not load EndNote file from scholar. Try again? [Y/N]').lower()
                if result == "y":
                    result = True
                elif result == "n":
                    result = False
    ...
    if "pages" in EndNote_info:
        try:
            ...
            if len(pages) == 2:
                ...
                if re_st_page:
                    EndNote_info["start_page"] = int(re_st_page.group())
                if re_end_page:
                    EndNote_info["end_page"] = int(re_end_page.group())
                if re_st_page and re_end_page:
                    EndNote_info["volume"] = EndNote_info["end_page"] - \
...
def _search_scholar_soup(soup, handling_cluster,
                         max_papers_count, total_papers, start_paper):
    ...
    while True:
        ...
        logger.debug(
            "Find papers on page #{0} (max_google_papers = {1})".format(
        ...
        if soup.find(
            ...
            while result and soup is None:
                ...
                if soup is None:
                    ...
                    while result is None:
                        result = input(
                            'Do not load new page on scholar. Try again? [Y/N]').lower()
                        if result == "y":
                            result = True
                        elif result == "n":
                            result = False
...
def search_pubs_query_with_control_params(params):
    ...
    return search_pubs_query_with_params(
        ...
        (True if params['words_in_body'] ==
         'true' else False) if 'words_in_body' in params else True,
        (True if params['patents'] ==
         'true' else False) if 'patents' in params else True,
        (True if params['citations'] ==
         'true' else False) if 'citations' in params else True,
        params["google_clusters_handling"].lower(
        ) == "true" if "google_clusters_handling" in params else False,
        int(params["max_google_papers"]) if "max_google_papers" in params else float(
            "inf") if "max_google_papers" in params else float("inf"),
...
def search_pubs_custom_url(url, handling_cluster, max_iter, start_paper):
    ...
    return (_search_scholar_soup(soup, handling_cluster,
                                 max_iter, about, start_paper), about)
