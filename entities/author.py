...
class Author(object):
    ...
    def get_base_info_from_sch(self, info_dict):
        if "name" in info_dict: self.name = info_dict["name"]
        if "shortname" in info_dict: self.shortname = info_dict["shortname"]
        if "gid" in info_dict and info_dict["gid"] != '': self.g_id = info_dict["gid"]
