        ...
        for paper_info in paper_generator:
            ...
            for paper_version_counter, paper_addition_information in enumerate(paper_info["different_information"]):
                ...
                logger.debug("Process content of EndNote file #%i\n%s\n%s" % (
                    papers_counter, json.dumps(paper_info["general_information"]),
                ...
                if newpaper.in_database():
                    settings.print_message("This paper%s already exists, id = %i." % (
                    (" version" if paper_versions > 1 else ""), newpaper.db_id), 1)
                else:
...
<<<<<<< HEAD
                        logger.debug("Failed get_pdf from Google Scholar for paper #{0}. URL={0}".format(new_papers - 1,
                                                                                                         url_for_download_from_gs))
                        settings.print_message(
                            "failed load PDF on Google Scholar. URL={0}".format(url_for_download_from_gs), 2)
                    # continue
=======
