...
import spliter3
import cnn
from settings import print_message, LOG_LEVEL, MAIN_DIR
...
def del_from_temp():
    ...
    for fn in os.listdir(f'{MAIN_DIR}/captcha/tmp'):
        fn = f'{MAIN_DIR}/captcha/tmp/{fn}'
        os.remove(fn)
    return len_for_del


def load_chars():
    for fn in sorted(os.listdir(f'{MAIN_DIR}/captcha/tmp')):
        try:
            fn = f'{MAIN_DIR}/captcha/tmp/{fn}'
            x = cv2.imread(fn)
...
def predict_imgs(fns):
    ...
    for kk, fn in enumerate(fns):
        ...
        for i in range(len(chars)):
            cv2.imwrite(f'{MAIN_DIR}/captcha/tmp/{i}.jpg', chars[i])
        ans_list = []
        for char in load_chars():
            ans = class_dict[model.predict_classes(char)[0]]
            ans_list.append(ans)
        ans = ''.join(ans_list)
        shutil.copyfile(fn, f'{MAIN_DIR}/captcha/nn_ans/{ans}.jpg')
