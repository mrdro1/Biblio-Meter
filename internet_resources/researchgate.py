...
def identification_and_fill_paper(params, query_soup = None, delay=0):
    ...
    try:
        ...
        paper_info_url = _ident_and_fill_paper(
            get_query_soup(params) if query_soup == None else query_soup, params)
...
def _ident_and_fill_paper(soup, params):
    ...
    while True:
        ...
        if soup.find('div', class_='search-noresults-headline') != None:
        ...
        for papers_item in papers_box:
            ...
            try:
                ...
                year = int(papers_item.find('div', class_='publication-metadata').find('span').text.split()[1])
                title = papers_item.find("a", class_="publication-title").text.strip().lower()
                type = papers_item.find('div', class_='publication-type').text.strip().lower()
                logger.debug("Process paper #%i (title='%s'; year=%i; auth_count=%i; type='%s')" % (papers_count, title, year, authors, type))
