...
class Paper(object):
    ...
    def get_info_from_sch(self, general_information,
                          additional_information, paper_version=1, pdf_url=None):
        # General
        if "title" in general_information:
            self.title = general_information["title"]
        if "year" in general_information:
            self.year = general_information["year"]
        if "cluster" in general_information:
            self.cluster = general_information["cluster"]
        if "url" in general_information:
            self.paper_URL = general_information["url"]
        self.PDF_URL = pdf_url
        # Addition
        self.paper_version = paper_version
        if "title" in additional_information:
            self.title = additional_information["title"]
        if "citedby" in additional_information:
            self.citedby = additional_information["citedby"]
        if "versions" in additional_information:
            self.versions = additional_information["versions"]
        if "url_scholarbib" in additional_information:
            self.EndNoteURL = additional_information["url_scholarbib"]
        if "year" in additional_information:
            self.year = int(additional_information["year"])
        if "type" in additional_information:
            self.google_type = additional_information["type"]
        if "publisher" in additional_information:
            self.publisher = additional_information["publisher"]
        if "secondarytitle" in additional_information:
            self.secondaryTitle = additional_information["secondarytitle"]
        if "pages" in additional_information:
            self.pages = additional_information["pages"]
        ...
        if "EndNote" in additional_information:
            self.EndNote = additional_information["EndNote"]
    ...
    def make_EndNote(self, replace_EndNote=True):
        ...
        EndNote = \
            ...
            (param_template.format(
                param=EndNote_params["Abstract"], value=self.abstract) if self.abstract else "")
        for author in self.authors:
            EndNote += param_template.format(
                param=EndNote_params["Author"], value=author)
        if replace_EndNote:
    ...
    def is_downloaded(self):
        param = {"id": self.db_id}
        self.downloaded = dbutils.get_pdf_download_transaction(
            param) is not None
